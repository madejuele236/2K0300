...
void SampleOperatorInputs()
{
    key1_state=primer::port::presentation::DigitalKeyAt(0).get_level();
    key2_state=primer::port::presentation::DigitalKeyAt(1).get_level();
    key3_state=primer::port::presentation::DigitalKeyAt(2).get_level();
    key4_state=primer::port::presentation::DigitalKeyAt(3).get_level();
    key5_state =primer::port::presentation::DigitalKeyAt(4).get_level();
    key6_state =primer::port::presentation::DigitalKeyAt(5).get_level();
    key7_state = primer::port::presentation::AnalogKeyAt(0).convert();
    key8_state = primer::port::presentation::AnalogKeyAt(1).convert();
}

void ApplyNavigationInputs()
{
    ...
    if(key4_state==0&&key4_last_state==1)
    {
...
void ApplyRunCommandInputs()
{
    if(key5_state==0&&key5_last_state==1)
    { primer::port::presentation::SetRunFlag(2); }
    if(key6_state==0&&key6_last_state==1)
    {
        ...
        primer::port::presentation::SetRunFlag(1);
    }
}

void ApplyParameterAdjustmentInputs()
{
    if(key7_state<4000&&key7_last_state>=4000)
    {
    ...
    if(key8_state<4000&&key8_last_state>=4000)
    {
        if(Parameter_flag == 1&&oled_flag==4){primer::port::presentation::ObserveParameters().debug_rgb_r_min+=5;}
        if(Parameter_flag == 2&&oled_flag==4){primer::port::presentation::ObserveParameters().debug_rgb_rg_diff+=5;}
        if(Parameter_flag == 3&&oled_flag==4){primer::port::presentation::ObserveParameters().debug_rgb_rb_diff+=5;}
...
{
    SampleOperatorInputs();
    ApplyNavigationInputs();
    ApplyRunCommandInputs();
    ApplyParameterAdjustmentInputs();
    LatchOperatorInputs();
}
