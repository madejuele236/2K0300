...
// }
std::string LQ_NCNN::Infer(const cv::Mat& bgr_image,float &confidence)
{
    if (!m_initialized) {
        throw std::runtime_error("NCNN not initialized. Call Init() first.");
    }
    ...
    // 获取最大概率类别
    int class_id = Argmax(logits, confidence);
    if (class_id < 0) {
        throw std::runtime_error("Failed to get class id from output logits.");
    }
    // printf("Predict: %s | Confidence: %.2f%%\n",
    // m_labels[class_id].c_str(),
    // confidence);
    // 返回类别名称或编号
    if (class_id >= 0 && class_id < static_cast<int>(m_labels.size())) {
        return m_labels[class_id];
...
int LQ_NCNN::Argmax(const ncnn::Mat& logits, float& confidence)
{
    if (logits.w <= 0) {
        confidence = 0.0f;
        return -1;
